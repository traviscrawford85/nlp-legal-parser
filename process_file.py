...
def build_summary(entities, filename, doc_type_from_nlp):
    ...
    # Filter clean party names
    def is_real_name(name):
        if len(name.split()) < 2:
            return False
        if any(term.lower() in name.lower() for term in ["bad faith", "due process", "background", "pro se"]):
            return False
        if name.isupper() or name.istitle() is False:
            return False
        return True

    # Prioritize PERSON over ORG, but filter both
    candidates = []
    for label in ["PERSON", "ORG"]:
        candidates += [name for name in entities.get(label, []) if is_real_name(name)]
        if len(candidates) >= 2:
            break

    parties = candidates[:2]

    # Pick the earliest valid filing date
    filing_date = None
    parsed_dates = []
...
if __name__ == "__main__":
    ...
    process_document(args.file, rename=args.rename)
